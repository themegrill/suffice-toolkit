...
abstract class ST_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		foreach ( $this->settings as $key => $setting ) {
			...
			if ( ! in_array( $group_name, $group_name_array ) ) {
				...
				if ( $group_name_array[0] != '' ) {
					?>
					<a class="suffice-tab-title <?php echo ( $groupcount == 1 ? ' active' : '' ); ?>" href="#suffice-tab-<?php echo esc_attr( $groupcount ); ?>">
		...
		foreach ( $group_name_array as $group ) {
			?>
			<div class="suffice-toolkit-tab" id="suffice-tab-<?php echo esc_attr( $groupcount ); ?>">
						...
						switch ( $setting['type'] ) {
							...
							<p class="suffice-radio-image st-widget-col <?php echo esc_attr( $field_width ); ?>">
								...
								<span class="suffice-radio-image-options">
									<?php foreach ( $setting['options'] as $option_key => $option_image ) : ?>
										<label>
											<input type="radio" <?php checked( $option_key, $value ); ?> id="<?php echo esc_attr( $this->get_field_id( $option_key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" value="<?php echo esc_attr( $option_key ); ?>"/>
							...
							<div id="tg-widget-image-uploader" class="suffice-media st-widget-col <?php echo esc_attr( $class ) . ' ' . esc_attr( $field_width ); ?>">
								...
								<div class="media-uploader" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>">
									...
									<p>
										<input type="text" class="widefat tg-media-input" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" value="<?php echo esc_url( $value ); ?>" style="margin-top:5px;" />
										<button class="tg-image-upload button button-secondary button-large" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" data-choose="<?php esc_attr_e( 'Choose an image', 'suffice-toolkit' ); ?>" data-update="<?php esc_attr_e( 'Use image', 'suffice-toolkit' ); ?>" style="width:100%;margin-top:6px;margin-right:30px;"><?php esc_html_e( 'Select an Image', 'suffice-toolkit' ); ?></button>
									</p>
								</div>
							</div>
									<?php
							...
							<div id="tg-widget-icon-picker" class="suffice-icon st-widget-col <?php echo esc_attr( $class ) . ' ' . esc_attr( $field_width ); ?>">
								<p><label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $setting['label'] ); ?></label></p>
								<select class="widefat suffice-enhanced-select-icons" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" data-placeholder="<?php esc_attr_e( 'Choose icons&hellip;', 'suffice-toolkit' ); ?>" title="<?php esc_attr_e( 'Icon', 'suffice-toolkit' ); ?>" style="width: 100%">
									<option value=""></option>
									<?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
							...
							<p class="st-widget-col <?php echo esc_attr( $field_width ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $setting['label'] ); ?></label>
								<input class="widefat suffice-toolkit-color-picker <?php echo esc_attr( $class ); ?>" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" type="text" value="<?php echo esc_attr( $value ); ?>" />
							</p>
									<?php
								break;
							...
							<p class="st-widget-col <?php echo esc_attr( $field_width ); ?>">
								<label for="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>"><?php echo esc_html( $setting['label'] ); ?></label>
								<select class="widefat suffice-enhanced-select-fonts" id="<?php echo esc_attr( $this->get_field_id( $key ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( $key ) ); ?>" data-placeholder="<?php esc_attr_e( 'Choose fonts&hellip;', 'suffice-toolkit' ); ?>" title="<?php esc_attr_e( 'Font', 'suffice-toolkit' ); ?>" style="width: 100%">
									<option value=""></option>
									<?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
										<option value="<?php echo esc_attr( $option_key ); ?>" <?php selected( $option_key, $value ); ?>><?php echo esc_html( $option_value ); ?></option>
									<?php endforeach; ?>
							...
							case 'repeater':
								include __DIR__ . '/views/html-admin-tmpl-repeater.php';
								break;
			...
			switch ( $setting['type'] ) {
				case 'text':
					...
					<p>
						...
						<input class="widefat <?php echo esc_attr( $class ); ?>" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_name ); ?>" type="text" value="<?php echo esc_attr( $value ); ?>" />
					</p>
					<?php
					break;
				case 'number':
					...
					<p>
						<label for="<?php echo esc_attr( $field_id ); ?>"><?php echo esc_html( $setting['label'] ); ?></label>
						<input class="widefat <?php echo esc_attr( $class ); ?>" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_name ); ?>" type="number" step="<?php echo esc_attr( $setting['step'] ); ?>" min="<?php echo esc_attr( $setting['min'] ); ?>" max="<?php echo esc_attr( $setting['max'] ); ?>" value="<?php echo esc_attr( $value ); ?>" />
					</p>
					<?php
					break;
				case 'select':
					...
					<p>
						<label for="<?php echo esc_attr( $field_id ); ?>"><?php echo esc_html( $setting['label'] ); ?></label>
						<select class="widefat <?php echo esc_attr( $class ); ?>" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_name ); ?>">
							<?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
								<option value="<?php echo esc_attr( $option_key ); ?>" <?php selected( $option_key, $value ); ?>><?php echo esc_html( $option_value ); ?></option>
							<?php endforeach; ?>
						</select>
				...
				case 'textarea':
					...
					<p>
						<label for="<?php echo esc_attr( $field_id ); ?>"><?php echo esc_html( $setting['label'] ); ?></label>
						<textarea class="widefat <?php echo esc_attr( $class ); ?>" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_name ); ?>" cols="20" rows="3"><?php echo esc_textarea( $value ); ?></textarea>
						<?php if ( isset( $setting['desc'] ) ) : ?>
							<small><?php echo esc_html( $setting['desc'] ); ?></small>
						<?php endif; ?>
					</p>
				...
				case 'checkbox':
					?>
					<p>
						<input class="checkbox <?php echo esc_attr( $class ); ?>" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_name ); ?>" type="checkbox" value="1" <?php checked( $value, 1 ); ?> />
						<label for="<?php echo esc_attr( $field_id ); ?>"><?php echo esc_html( $setting['label'] ); ?></label>
					</p>
					<?php
					break;
				case 'image':
					...
					<div id="tg-widget-image-uploader" class="suffice-media <?php echo esc_attr( $class ); ?>">
						<p><label for="<?php echo esc_attr( $field_id ); ?>"><?php echo esc_html( $setting['label'] ); ?></label></p>
						<div class="media-uploader" id="<?php echo esc_attr( $field_id ); ?>">
							<div class="tg-media-preview">
								<button class="tg-media-remove dashicons dashicons-no-alt">
									<span class="screen-reader-text"><?php esc_html_e( 'Remove media', 'suffice-toolkit' ); ?></span>
								</button>
							...
							<p>
								<input type="text" class="widefat tg-media-input" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_name ); ?>" value="<?php echo esc_url( $value ); ?>" style="margin-top:5px;" />
								<button class="tg-image-upload button button-secondary button-large" id="<?php echo esc_attr( $field_id ); ?>" data-choose="<?php esc_attr_e( 'Choose an image', 'suffice-toolkit' ); ?>" data-update="<?php esc_attr_e( 'Use image', 'suffice-toolkit' ); ?>" style="width:100%;margin-top:6px;margin-right:30px;"><?php esc_html_e( 'Select an Image', 'suffice-toolkit' ); ?></button>
							</p>
						</div>
					</div>
					<?php
				...
				case 'icon_picker':
					...
					<div id="tg-widget-icon-picker" class="suffice-icon <?php echo esc_attr( $class ); ?>">
						<p><label for="<?php echo esc_attr( $field_id ); ?>"><?php echo esc_html( $setting['label'] ); ?></label></p>
						<select class="widefat suffice-enhanced-select-icons" id="<?php echo esc_attr( $field_id ); ?>" name="<?php echo esc_attr( $field_name ); ?>" data-placeholder="<?php esc_attr_e( 'Choose icons&hellip;', 'suffice-toolkit' ); ?>" title="<?php esc_attr_e( 'Icon', 'suffice-toolkit' ); ?>" style="width: 100%">
							<option value=""></option>
							<?php foreach ( $setting['options'] as $option_key => $option_value ) : ?>
								<option value="<?php echo esc_attr( $option_key ); ?>" data-icon="<?php echo esc_attr( $option_key ); ?>" <?php selected( $option_key, $value ); ?>><?php echo esc_html( $option_value ); ?></option>
							<?php endforeach; ?>
